...
class DocutilsValidator extends node_events_1.EventEmitter {
    ...
    async validateTypeDoc() {
        ...
        const typeDocPath = this.typeDocPath ?? (await (0, fs_1.findTypeDoc)(pkgDir));
        if (!typeDocPath) {
            return this.fail(`Could not find ${constants_1.NAME_TYPEDOC}; is it installed?`);
        }
        log.debug('Found %s at %s', constants_1.NAME_TYPEDOC, typeDocPath);
        let rawTypeDocVersion;
        let typeDocVersion;
        try {
            ({ stdout: rawTypeDocVersion } = await (0, teen_process_1.exec)(process.execPath, [typeDocPath, '--version'], {
                cwd: pkgDir,
            }));
        }
        catch (err) {
            return this.fail(`Could not execute ${process.execPath} ${typeDocPath} from ${pkgDir}. Reason: ${err.message}`);
        }
        if (rawTypeDocVersion) {
            ...
            const reqdTypeDocVersion = constants_1.DOCUTILS_PKG.dependencies.typedoc; // this is our own package.json
